...
As an example, we can observe that the CNV with **cnv.id**=3 has 4 variants matching 
...
plot easily the duplication CNV with **cnv.id**=3 and all the variants matching it.

```{r, fig.width=8, fig.height=4, fig.wide=TRUE}
plotVariantsForCNV(results, "3")
```

We can do the same to plot the deletion CNV with **cnv.id**=19, where all variants
discard the CNV except one homozygous variant that does not give us any 
