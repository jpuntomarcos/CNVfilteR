---
title: "CNVfilteR: Remove false positives of any CNV calling tools by using SNV calls "
...
variants loading, identifying false postives and plotting the results.
...
As an example, we can observe that the CNV with `cnv.id`=3 has 4 variants 
...
plot easily the duplication CNV with `cnv.id`=3 and all the variants matching
it.
...
For example, the CNV with `cnv.id`=14 has one variant matching it. If we get
...
`r BiocStyle::Biocpkg("CNVfilteR")` functions expect CNVs calls to be a
...
Common NGS pipelines produce variant calling (SNVs or INDELs) in a VCF file.
However, VCF interpretation is 
...
when identifying those CNVs. `loadVCFs` will load only 
variants with a minimum total depth
greater than `min.total.depth` (defaults to 30). Additionally, we can exclude
those regions that have already known artifacts with the parameter 
`regions.to.exclude`. In this example, we exclude PMS2, PRSS1, and
...
each CNV present in `cnvs.gr` to identify those CNVs that can be filtered. It
 ...
 * A **deletion CNV** can be filtered if there is at least 
 `ht.deletions.threshold`% of heterozygous variants matching
 the CNV. Default `ht.deletions.threshold` value is 15, so 15% is required.
 * A **duplication CNV** can be filtered if the total `score` is >= 
 `dup.threshold.score` after computing all 
 heterozygous variants matching that CNV. Default `dup.threshold.score` value
...
discarded
is based on the allele frequency for each heterozygous variant matching
that CNV:
...
The total `score` is computed among all the variants matching the CNV. If the 
